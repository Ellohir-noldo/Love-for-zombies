...
function TEsound.play(sound, tags, volume, pitch, func)
        if TEsound.disable then return end
...
function TEsound.disable_sound()
    TEsound.disable = true
    for k,v in ipairs(TEsound.channels) do
        TEsound.channels[k][1]:stop()
    end
end

function TEsound.enable_sound()
    TEsound.disable = false
end
